...
for batch_size in "${batch_sizes[@]}"; do
  ...
  nvidia-smi --loop-ms=150 --format=csv --query-gpu=power.draw > "power_data_cnn_${batch_size}.log" &
  pid1=$!
  ...
  echo "Elapsed time (30 executions): $elapsed_time seconds" > "${batch_size}_cnn_elapsed_time.txt"

  kill $pid1

  if [ ! -f "power_data_cnn_${batch_size}.log" ]; then
  ...
  while read line; do
    ...
    count=$((count + 1))
  done < <(tail -n +2 "power_data_cnn_${batch_size}.log")

  # Calculate the average power
  average_power=$(echo "scale=2; $total_power / $count" | bc)

  # Print the result
  echo "Average power: $average_power W" >> "${batch_size}_cnn_elapsed_time.txt"
done
